...
class TestReassemblyBasic:
    ...
    async def test_single_chunk_reassembly(self) -> None:
        data = b"hello world"
        meta = _meta(len(data), chunk_size=1024)
        assembler = ChunkAssembler(meta)

        chunks = [c async for c in split(data, meta, transfer_id="s")]
        assert len(chunks) == 1
    ...
    async def test_multi_chunk_in_order(self) -> None:
    ...
    async def test_split_then_reassemble_round_trip(self) -> None:
...
class TestOutOfOrder:
    ...
    async def test_reverse_order(self) -> None:
        ...
        chunks = [c async for c in split(data, meta, transfer_id="rev")]
        # write in reverse
    ...
    async def test_interleaved_order(self) -> None:
        ...
        chunks = [c async for c in split(data, meta, transfer_id="inter")]
    ...
    async def test_random_order(self) -> None:
        import random
        ...
        chunks = [c async for c in split(data, meta, transfer_id="rand")]
        rng.shuffle(chunks)
    ...
    async def test_last_chunk_first(self) -> None:
        """Writing only the last (short) chunk first, then the rest."""
        data = b"Z" * 2500  # last chunk = 500 bytes
        meta = _meta(len(data), chunk_size=1000)
        assembler = ChunkAssembler(meta)

        chunks = [c async for c in split(data, meta, transfer_id="last-first")]
...
class TestDuplicateWrites:
    ...
    async def test_duplicate_is_idempotent(self) -> None:
        data = b"abc" * 500
        meta = _meta(len(data), chunk_size=500)
        assembler = ChunkAssembler(meta)

        chunks = [c async for c in split(data, meta, transfer_id="dup")]
        await assembler.write_chunk(chunks[0])
...
class TestProgress:
    ...
    async def test_progress_partial(self) -> None:
        meta = _meta(4000, chunk_size=1000)
        assembler = ChunkAssembler(meta)
        data = b"x" * 4000
        chunks = [c async for c in split(data, meta, transfer_id="prog")]
        await assembler.write_chunk(chunks[0])
        assert assembler.progress == pytest.approx(0.25)

    async def test_progress_complete(self) -> None:
    ...
    async def test_missing_seqs(self) -> None:
        meta = _meta(3000, chunk_size=1000)
        assembler = ChunkAssembler(meta)
        data = b"x" * 3000
        chunks = [c async for c in split(data, meta, transfer_id="miss")]
...
class TestReassemblyErrors:
    ...
    async def test_seq_out_of_range(self) -> None:
    ...
    async def test_negative_seq(self) -> None:
        meta = _meta(1024, chunk_size=1024)
        assembler = ChunkAssembler(meta)
