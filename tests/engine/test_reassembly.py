...
class TestReassemblyBasic:
    ...
    @pytest.mark.asyncio
    async def test_single_chunk_reassembly(self, tmp_path) -> None:
        data = b"hello world"
        meta = _meta(len(data), chunk_size=1024)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")

        chunks = await _collect(data, meta, transfer_id="s")
        assert len(chunks) == 1
    ...
    @pytest.mark.asyncio
    async def test_multi_chunk_in_order(self, tmp_path) -> None:
        data = b"A" * 1000 + b"B" * 1000 + b"C" * 500
        meta = _meta(len(data), chunk_size=1000)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")

        async for chunk in split(data, meta, transfer_id="s"):
    ...
    @pytest.mark.asyncio
    async def test_split_then_reassemble_round_trip(self, tmp_path) -> None:
        """split + reassemble should be a perfect round-trip."""
        data = bytes(range(256)) * 10  # 2560 bytes
        meta = _meta(len(data), chunk_size=700)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")

        async for chunk in split(data, meta, transfer_id="rt"):
            await assembler.write_chunk(chunk)
...
class TestOutOfOrder:
    ...
    @pytest.mark.asyncio
    async def test_reverse_order(self, tmp_path) -> None:
        data = b"A" * 1000 + b"B" * 1000 + b"C" * 1000 + b"D" * 500
        meta = _meta(len(data), chunk_size=1000)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")

        chunks = await _collect(data, meta, transfer_id="rev")
        # write in reverse
    ...
    @pytest.mark.asyncio
    async def test_interleaved_order(self, tmp_path) -> None:
        data = b"x" * 5000
        meta = _meta(len(data), chunk_size=1000)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")

        chunks = await _collect(data, meta, transfer_id="inter")
    ...
    @pytest.mark.asyncio
    async def test_random_order(self, tmp_path) -> None:
        import random
        ...
        chunks = await _collect(data, meta, transfer_id="rand")
        rng.shuffle(chunks)
    ...
    @pytest.mark.asyncio
    async def test_last_chunk_first(self, tmp_path) -> None:
        """Writing only the last (short) chunk first, then the rest."""
        data = b"Z" * 2500  # last chunk = 500 bytes
        meta = _meta(len(data), chunk_size=1000)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")

        chunks = await _collect(data, meta, transfer_id="last-first")
        # write last chunk first
        await assembler.write_chunk(chunks[-1])
...
class TestDuplicateWrites:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_is_idempotent(self, tmp_path) -> None:
        data = b"abc" * 500
        meta = _meta(len(data), chunk_size=500)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")

        chunks = await _collect(data, meta, transfer_id="dup")
        await assembler.write_chunk(chunks[0])
        await assembler.write_chunk(chunks[0])  # duplicate
...
class TestProgress:
    ...
    @pytest.mark.asyncio
    async def test_progress_starts_at_zero(self, tmp_path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_progress_partial(self, tmp_path) -> None:
        meta = _meta(4000, chunk_size=1000)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")
        data = b"x" * 4000
        chunks = await _collect(data, meta, transfer_id="prog")
        await assembler.write_chunk(chunks[0])
        assert assembler.progress == pytest.approx(0.25)
        assembler.close()

    @pytest.mark.asyncio
    async def test_progress_complete(self, tmp_path) -> None:
        meta = _meta(2000, chunk_size=1000)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")
        data = b"x" * 2000
        async for chunk in split(data, meta, transfer_id="comp"):
            await assembler.write_chunk(chunk)
        assert assembler.progress == 1.0
        assembler.close()

    @pytest.mark.asyncio
    async def test_missing_seqs(self, tmp_path) -> None:
        meta = _meta(3000, chunk_size=1000)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")
        data = b"x" * 3000
        chunks = await _collect(data, meta, transfer_id="miss")
        await assembler.write_chunk(chunks[1])  # only write seq=1
        assert assembler.missing_seqs == {0, 2}
        assembler.close()

    @pytest.mark.asyncio
    async def test_empty_file_progress(self, tmp_path) -> None:
        meta = _meta(0)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")
        assert assembler.progress == 1.0
...
class TestReassemblyErrors:
    ...
    @pytest.mark.asyncio
    async def test_hash_mismatch_rejected(self, tmp_path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_seq_out_of_range(self, tmp_path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_negative_seq(self, tmp_path) -> None:
        meta = _meta(1024, chunk_size=1024)
        assembler = ChunkAssembler(meta, tmp_path / "test.bin")
    ...
    @pytest.mark.asyncio
    async def test_to_bytes_before_complete_raises(self, tmp_path) -> None:
