...
class TestSplitBasic:
    ...
    def test_multiple_chunks(self) -> None:
        ...
        data = b"a" * 4096
        meta = _meta(len(data), chunk_size=1024)
        chunks = list(split(data, meta, transfer_id="sess-2"))
        assert len(chunks) == 4
...
class TestSplit1KBEdge:
    ...
    the default 256 KB chunk size, producing a single partial chunk."""

    def test_1kb_file_single_chunk(self) -> None:
        """A 1024-byte file with default 256KB chunk_size => 1 chunk."""
        data = b"\xAB" * 1024
        meta = _meta(len(data))  # default 256KB chunk_size
        chunks = list(split(data, meta, transfer_id="1kb-test"))
        assert len(chunks) == 1
        assert chunks[0].data == data
        assert len(chunks[0].data) == 1024

    def test_1kb_chunk_is_smaller_than_chunk_size(self) -> None:
        """The single chunk should be smaller than chunk_size (partial)."""
        data = b"\xCD" * 1024
        meta = _meta(len(data))
        chunks = list(split(data, meta, transfer_id="1kb-partial"))
        assert len(chunks[0].data) < meta.chunk_size

    def test_1kb_sha256_is_correct(self) -> None:
        """The chunk's sha256 should match a manual computation."""
        data = b"\xEF" * 1024
        meta = _meta(len(data))
        chunks = list(split(data, meta, transfer_id="1kb-hash"))
...
class TestLastChunkSize:
    ...
    def test_last_chunk_smaller(self) -> None:
        """When file size isn't a multiple of chunk_size, the last chunk is short."""
        data = b"d" * 2500
        meta = _meta(len(data), chunk_size=1000)
        chunks = list(split(data, meta, transfer_id="last-short"))
        assert len(chunks) == 3
    ...
    def test_one_byte_over(self) -> None:
        ...
        data = b"e" * 1025
        meta = _meta(len(data), chunk_size=1024)
        chunks = list(split(data, meta, transfer_id="over"))
        assert len(chunks) == 2
        assert len(chunks[0].data) == 1024
...
class TestChunkSha256:
    ...
    def test_per_chunk_hash(self) -> None:
        data = b"A" * 1000 + b"B" * 1000 + b"C" * 500
        meta = _meta(len(data), chunk_size=1000)
        chunks = list(split(data, meta, transfer_id="hash-check"))
    ...
    def test_different_data_different_hash(self) -> None:
        ...
        chunks = list(split(data, meta, transfer_id="diff"))
...
class TestEmptyFile:
    ...
    def test_empty_produces_nothing(self) -> None:
        data = b""
        meta = _meta(0)
        chunks = list(split(data, meta, transfer_id="empty"))
        assert len(chunks) == 0
...
class TestChunkCountMatchesMetadata:
    ...
    def test_exact_multiple(self) -> None:
        data = b"x" * 4096
        meta = _meta(len(data), chunk_size=1024)
        chunks = list(split(data, meta, transfer_id="exact"))
        assert len(chunks) == meta.chunk_count == 4

    def test_partial(self) -> None:
        data = b"x" * 3000
        meta = _meta(len(data), chunk_size=1024)
        chunks = list(split(data, meta, transfer_id="partial"))
        assert len(chunks) == meta.chunk_count == 3

    def test_tiny_chunk_size(self) -> None:
        """With chunk_size=1, each byte becomes its own chunk."""
        data = b"hello"
        meta = _meta(len(data), chunk_size=1)
        chunks = list(split(data, meta, transfer_id="tiny"))
        assert len(chunks) == 5
