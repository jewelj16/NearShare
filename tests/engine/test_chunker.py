...
class TestSplitBasic:
    ...
    async def test_single_full_chunk(self) -> None:
        ...
        chunks = await _collect(data, meta, transfer_id="sess-1")
        assert len(chunks) == 1
    ...
    async def test_multiple_chunks(self) -> None:
        ...
        data = b"a" * 4096
        meta = _meta(len(data), chunk_size=1024)
        chunks = await _collect(data, meta, transfer_id="sess-2")
        assert len(chunks) == 4
    ...
    async def test_reassembly(self) -> None:
        ...
        chunks = await _collect(data, meta, transfer_id="sess-3")
    ...
    async def test_transfer_id_and_file_index(self) -> None:
        ...
        chunks = await _collect(data, meta, transfer_id="t-1", file_index=7)
        for chunk in chunks:
    ...
    async def test_is_async_generator(self) -> None:
        """split() returns an async generator — streaming friendly."""
        data = b"x" * 100
        meta = _meta(len(data), chunk_size=10)
        result = split(data, meta, transfer_id="s")
        # should be an async generator, not a list
...
class TestSplit1KBEdge:
    ...
    async def test_1kb_file_single_chunk(self) -> None:
        ...
        chunks = await _collect(data, meta, transfer_id="1kb-test")
        assert len(chunks) == 1
        assert chunks[0].data == data
    ...
    async def test_1kb_chunk_is_smaller_than_chunk_size(self) -> None:
        ...
        data = b"\xCD" * 1024
        meta = _meta(len(data))
        chunks = await _collect(data, meta, transfer_id="1kb-partial")
    ...
    async def test_1kb_sha256_is_correct(self) -> None:
        ...
        data = b"\xEF" * 1024
        meta = _meta(len(data))
        chunks = await _collect(data, meta, transfer_id="1kb-hash")
...
class TestLastChunkSize:
    ...
    async def test_last_chunk_smaller(self) -> None:
        ...
        data = b"d" * 2500
        meta = _meta(len(data), chunk_size=1000)
        chunks = await _collect(data, meta, transfer_id="last-short")
        assert len(chunks) == 3
    ...
    async def test_one_byte_over(self) -> None:
        ...
        data = b"e" * 1025
        meta = _meta(len(data), chunk_size=1024)
        chunks = await _collect(data, meta, transfer_id="over")
        assert len(chunks) == 2
        assert len(chunks[0].data) == 1024
...
class TestChunkSha256:
    ...
    async def test_per_chunk_hash(self) -> None:
        ...
        meta = _meta(len(data), chunk_size=1000)
        chunks = await _collect(data, meta, transfer_id="hash-check")
    ...
    async def test_different_data_different_hash(self) -> None:
        ...
        chunks = await _collect(data, meta, transfer_id="diff")
...
class TestEmptyFile:
    ...
    async def test_empty_produces_nothing(self) -> None:
        data = b""
        meta = _meta(0)
        chunks = await _collect(data, meta, transfer_id="empty")
        assert len(chunks) == 0
...
class TestChunkCountMatchesMetadata:
    ...
    async def test_exact_multiple(self) -> None:
        data = b"x" * 4096
        meta = _meta(len(data), chunk_size=1024)
        chunks = await _collect(data, meta, transfer_id="exact")
    ...
    async def test_partial(self) -> None:
        data = b"x" * 3000
        meta = _meta(len(data), chunk_size=1024)
        chunks = await _collect(data, meta, transfer_id="partial")
    ...
    async def test_tiny_chunk_size(self) -> None:
        ...
        chunks = await _collect(data, meta, transfer_id="tiny")
        assert len(chunks) == 5
